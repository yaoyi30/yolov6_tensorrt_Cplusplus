...
YOLOv6::YOLOv6(const std::string &config_file,const std::string &label_file) {
...
std::vector<YOLOv6::DetectRes> YOLOv6::Inference(cv::Mat &src_img) {
...
std::vector<YOLOv6::DetectRes> YOLOv6::EngineInference(cv::Mat &src_img, const int &outSize, void **buffers,
    ...
    std::vector<YOLOv6::DetectRes> boxes = postProcess(src_img, out, outSize);
    
    return boxes;
}

std::vector<float> YOLOv6::prepareImage(cv::Mat &src_img) {
...
std::vector<YOLOv6::DetectRes> YOLOv6::postProcess(const cv::Mat &src_img, float *output,
                                                                const int &outSize) {
    ...
    {
        for (int h = 0; h < grids[n][0]; h++)
        {
            for (int w = 0; w < grids[n][1]; w++)
            {
